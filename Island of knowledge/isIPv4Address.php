 ...
 * For inputString = "172.316.254.1", the output should be isIPv4Address(inputString) = false. 316 is not in range [0, 255].
 * For inputString = ".254.255.0", the output should be isIPv4Address(inputString) = false. There is no first number.
 */
function isIPv4Address($inputString) {
    $ipArray = explode('.', $inputString);
    ...
    foreach($ipArray as $ip) {
        if( strlen($ip) === 0 || (preg_match('/\D/', $ip, $matches) > 0) || $ip < 0 || $ip > 255) {
            return false;
        }
    }
